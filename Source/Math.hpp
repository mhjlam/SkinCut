...
// True if x is within a margin of epsilon of y
bool Equal(float x, float y, float ep = EPSILON);

// True if s is between min and max within a margin of epsilon
bool Bound(float s, float minimum, float maximum, float ep = EPSILON);

// True if vectors are within a margin of epsilon of each other
bool Equal(const Vector3& v0, const Vector3& v1, float ep = EPSILON);
...
inline Matrix Matrix::CreatePerspectiveOffCenter(float left, float right, float bottom, float top, 
...
inline Matrix Matrix::CreateOrthographicOffCenter(float left, float right, float bottom, float top, 
												  float nearPlane, float farPlane) {
	using namespace DirectX;
	Matrix R;
	XMStoreFloat4x4(&R, XMMatrixOrthographicOffCenterRH(left, right, bottom, top, nearPlane, farPlane));
	return R;
}
...
namespace std {
	...
	template<> struct less<SkinCut::Math::Ray> {
		bool operator()(const SkinCut::Math::Ray& R1, const SkinCut::Math::Ray& R2) const {
			...
			if (R1.origin.z != R2.origin.z) {
				return R1.origin.z < R2.origin.z;
			}
