...
std::vector<Tester::Sample> Tester::CreateSamples(SampleLocations& locations, std::vector<float>& lengths, std::wstring setName) {
	...
	for (uint32_t i = 0; i < locations.size(); ++i) {
		float halfLength = 0.5f * lengths[i];

		Math::Vector2 p = locations[i].first;		// position
		Math::Vector2 d = locations[i].second;	// direction

		Math::Vector2 p0 = p - d * halfLength;
		Math::Vector2 p1 = p + d * halfLength;
...
void Tester::RunTest(std::shared_ptr<Cutter> cutter, std::vector<Sample>& samples, 
	                 Math::Vector2& resolution, Math::Vector2& window, 
					 Math::Matrix& projection, Math::Matrix& view) {
	for (auto& sample : samples) {
		std::array<long long, 5> stageTime = {}; // init values to zero

		for (uint32_t run = 0; run < (uint32_t)Constants::NUM_TEST_RUNS; ++run) {
			StopWatch sw;
		...
		for (uint32_t ti = 0; ti < stageTime.size(); ti++) {
			double stageTimeAvg = stageTime[ti] / (double)Constants::NUM_TEST_RUNS / 1000.0;
...
void Tester::Test(std::shared_ptr<Cutter> cutter, Math::Vector2 resolution, Math::Vector2 window) {
	Math::Matrix proj {
	...
	std::vector<std::pair<Math::Vector2, Math::Vector2>> sampleLocations = {
		std::make_pair(Math::Vector2(638.0f, 175.0f), Math::Vector2(0.999650240f, 0.026445773f)),
		std::make_pair(Math::Vector2(754.0f, 342.0f), Math::Vector2(-0.138322249f, 0.990387261f)),
		std::make_pair(Math::Vector2(618.0f, 618.0f), Math::Vector2(0.899437010f, 0.437050372f)),
		std::make_pair(Math::Vector2(692.0f, 375.0f), Math::Vector2(0.474099845f, 0.880471110f)),
		std::make_pair(Math::Vector2(582.0f, 346.0f), Math::Vector2(-0.651344180f, 0.758782387f)),
		std::make_pair(Math::Vector2(631.0f, 467.0f), Math::Vector2(0.978677809f, 0.205401510f))
	};
	...
	auto cutSamplesMed = CreateSamples(sampleLocations, cutSizeMed, L"medium");	// medium-sized cuts (50% of large)
